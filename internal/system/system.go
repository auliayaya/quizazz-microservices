...
func NewSystem(cfg config.AppConfig) (*System, error) {
	...
	if err := s.initJS(); err != nil {
		fmt.Println("Apakah Error disini ", err)
		return nil, err
	}
...
func (s *System) initJS() (err error) {
	fmt.Println("URL Nats ", s.cfg.Nats.URL)
	fmt.Println("URL Nats Stream ", s.cfg.Nats.Stream)
	...
	if err != nil {
		fmt.Println("Error disini kah  ", err)
		return err
	}
	...
	fmt.Println("Berarti err disini ", err)
